...
function generateCells(){
    ...
    for(let i = 0; i<7; i++){
        ...
        element.id = (i*7)+j;
        element.setAttribute('data-id', game_matrix[i][j].id);
        element.style.backgroundColor = game_matrix[i][j].contains.style;
        element.style.backgroundImage = `url(${game_matrix[i][j].contains.img})`
...
function fill_board_initial(){
    for(i = 0; i < game_matrix.length; i++){
       for (let j = 0; j < game_matrix[i].length; j++) {
            game_matrix[i][j].contains = spawn_element();
        }
    }
}
...
function getElementById(id) {
    for (let i = 0; i < game_matrix.length; i++) {
        for (let j = 0; j < game_matrix[i].length; j++) {
            if (game_matrix[i][j].id === id) {
                return game_matrix[i][j];
            }
        }
    }
    return null;
